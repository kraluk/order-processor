...
class UpsertOrdersUseCaseTest {
  ...
  private final UpsertOrdersUseCase useCase =
  ...
  void shouldUpsertGivenOrders() {
    // Given
    final var order1 =
        TestOrderBuilder.builder().value(Money.of(BigDecimal.valueOf(1), "PLN")).build();
    final var order2 =
        TestOrderBuilder.builder().value(Money.of(BigDecimal.valueOf(2), "PLN")).build();

    // When
    final var result = useCase
        .invoke(UpsertOrdersUseCase.Command.of(Stream.of(order1, order2)))
        .toList();
