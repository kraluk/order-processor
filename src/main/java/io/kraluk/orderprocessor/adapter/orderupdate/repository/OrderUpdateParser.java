...
class CsvOrderUpdateParser implements OrderUpdateParser {
  ...
  public Stream<OrderUpdate> parse(final OrderUpdateContent content) {
    ...
    return StreamOps
        .toStream(source)
        .map(IntermediateOrderUpdate::toDomain);
  }
}

// FEATURE: this record is only an intermediate data model to not use framework-specific annotations on the domain OrderUpdate class
// potentially such optimisation could be not needed, and the domain class could be used directly with jackson-annotations dependency
record IntermediateOrderUpdate(UUID businessId, BigDecimal value, String currency, String notes, Instant updatedAt) {
