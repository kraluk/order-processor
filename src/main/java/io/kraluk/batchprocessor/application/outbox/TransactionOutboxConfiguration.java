...
class TransactionOutboxConfiguration {
  ...
  @Bean
  TransactionOutbox transactionOutbox(final SpringTransactionManager springTransactionManager, final SpringInstantiator springInstantiator,
      final ObjectMapper mapper, final @Qualifier("outboxExecutor") ExecutorService outboxExecutor,
      final TransactionOutboxProperties properties, final MeterRegistry registry) {
    log.info("Creating TransactionOutbox with the following properties - '{}'", properties);

    return TransactionOutbox.builder().instantiator(springInstantiator).transactionManager(springTransactionManager)
        .persistor(DefaultPersistor.builder().dialect(Dialect.POSTGRESQL_9).tableName("transaction_outbox").writeLockTimeoutSeconds(1)
            .migrate(false).serializer(JacksonInvocationSerializer.builder().mapper(mapper).build()).build())
        .submitter(ExecutorSubmitter.builder().executor(outboxExecutor).logLevelWorkQueueSaturation(Level.INFO).build())
        .flushBatchSize(properties.flushBatchSize()).attemptFrequency(properties.attemptFrequency())
        .listener(new TransactionOutboxMetrics(registry)).build();
  }
}
