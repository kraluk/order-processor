...
public class LocalStackOps {
  ...
  public void createBucket(final String bucketName) throws IOException, InterruptedException, IllegalStateException {
  ...
  public void deleteBucket(final String bucketName) throws IOException, InterruptedException {
    final var result = localstack.execInContainer("awslocal", "s3", "rb", "s3://" + bucketName, "--force");
  ...
  public void createQueue(final String queueName) throws IOException, InterruptedException {
    final var result = localstack.execInContainer("awslocal", "sqs", "create-queue", "--queue-name", queueName);
  ...
  public void deleteQueue(final String queueName) throws IOException, InterruptedException {
    final var result = localstack.execInContainer("awslocal", "sqs", "delete-queue", "--queue-url", queueUrl(queueName));
  ...
  private void handleResult(final Container.ExecResult result, final String process, final String resourceName) {
    if (result.getExitCode() == 0) {
      log.debug("{} for '{}' ended successfully with result - '{}'", process, resourceName, result.getStdout());
    } else {
      throw new IllegalStateException(
          format("Failed to '%s' for '%s' with result - '%s - %s'", process, resourceName, result.getExitCode(), result.getStderr()));
    }
  }
}